...
\subsection{Unterpunkte}\label{labUnterpunkte}
...
Die Syntax ist ähnlich zu der der Unterpunkte (siehe Abschnitt~\ref{labUnterpunkte}). Codesequenzen und Unterpunkte lassen sich auch innerhalb desselben Ausgabeeintrags kombinieren, die Ausgabe erfolgt jedoch blockweise, d.h.\ zuerst die Unterpunkte und dann der Codeblock.
...
\webbox{
\webheadR{Ergebnisliste (Demo, 21 Codegen)}\\[3mm]
Anleitung
\begin{numlist}
...
Im Beispiel wird ein Suchbegriff abgefragt und in die Variable \verb|SBeg| übertragen, da die Codierung nur für Parameter, nicht jedoch für Eingabe- und Frageschlüssel möglich ist. Die Variable wird anschließend in die URLs eingebaut und führt bei Eingabe von \verb|C# int?| zu folgender Ausgabe:
...
\subsection{Links mit Anzeigetext}
Aus Abschnitt~\ref{labUnterpunkte} wissen wir, dass Unterpunkte, die als URL interpretiert werden können, ausführbar ausgegeben werden. Die zuletzt gezeigten Möglichkeiten führen dabei jedoch in der Regel zu sehr langen und kryptischen Links, für die ein alternativer Anzeigetext wünscheswert wäre. Dies ist wie folgt möglich:
...
>> <Ausgabe Text>
    ...
    . . . 
\end{verbatim}
Der URL-Angabe wird, abgetrennt durch das Pipe-Zeichen der anzuzeigende Text nachgestellt. Wird der Zusatz weggelasen, so wird die URL selbst wie in Abschnitt~\ref{labUnterpunkte} beschrieben ausgegeben. Ist der vordere Teil nicht als URL interpretierbar, so wird der Anzeigetext ignoriert.
...
\webbox{
\webhead{Vorlage Demo, 26 Hilfstexte}{Hallo-Welt-Ausgabe mit Hilfestellung}
Wie soll \dq Hallo Welt\dq ausgegeben werden?
\begin{helplist}
...
\SysVar{MyResultURL}{Gibt die URL der aktuellen Vorlageausführung inklusive Schlüs\-sel\-pa\-ra\-me\-ter zurück, siehe auch Abschnitt~\ref{eigeneURL}.}
...
\subsection{Rangfolgenbestimmung}\label{labRangfolgenbestimmung}
