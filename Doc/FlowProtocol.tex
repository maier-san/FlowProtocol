...
\subsection{Parametrisiert Funktionen}
...
Spezialbefehle sind spezielle Befehle, die die Struktur einer Vorlage verändern, und die darauf erfolgten Eingaben in eigener Weise interpretieren und zu einem Ergebnis umsetzen können.
...
\end{verbatim}
Zu beachten ist, dass die \texttt{GroupName}-Angabe mit Strichpunkt abgeschlossen werden muss.
\Beispiel
\begin{verbatim}
...
\subsubsection*{Mögliche Fehlermeldungen}
\begin{description}
\FehlerBeschreibungItem{V01}{Vote-Befehl ohne Key-Argument.} Tritt auf, wenn das zwingende \texttt{Key}-Argument nicht angegeben wurde.
\FehlerBeschreibungItem{V02}{Frageschlüssel $\dots$ nicht gefunden.} Tritt auf, wenn der über das \texttt{Key}-Argument angegebene Frageschlüsselnicht angegeben wurde.
\FehlerBeschreibungItem{V03}{Vote-Befehl ohne gültiges $\dots$-Argument.} Tritt auf, wenn ein ungültiger Parameterwert angegeben wurde, z.B.\ eine \texttt{GroupSize}-Angabe, die nicht als Zahl interpretiert werden kann.
\end{description}
\end{document}